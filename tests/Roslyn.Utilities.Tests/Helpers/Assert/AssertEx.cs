...
namespace Roslyn.Utilities.Tests.Helpers.Assert
{
    ...
    public static class AssertEx
    {
        ...
        public static void Equal<T>(ImmutableArray<T> expected, IEnumerable<T> actual, IEqualityComparer<T> comparer = null, string message = null)
        {
            ...
            {
                Xunit.Assert.True((actual == null) == expected.IsDefault, message);
            }
            else
            {
        ...
        public static void Equal<T>(IEnumerable<T> expected, ImmutableArray<T> actual, IEqualityComparer<T> comparer = null, string message = null, string itemSeparator = null)
        {
            ...
            {
                Xunit.Assert.True((expected == null) == actual.IsDefault, message);
            }
            else
            {
            ...
            string itemSeparator = null, Func<T, string> itemInspector = null)
        {
            ...
            else if (!SequenceEqual(expected, actual, comparer))
            {
                ...
                Xunit.Assert.True(false, assertMessage);
            }
        }
        ...
        public static void SetEqual<T>(IEnumerable<T> actual, params T[] expected)
        {
            ...
            Xunit.Assert.True(expectedSet.SetEquals(actual), string.Format("Expected: {0}\nActual: {1}", ToString(expected), ToString(actual)));
        }
        ...
        public static void None<T>(IEnumerable<T> actual, Func<T, bool> predicate)
        {
            ...
            if (!none)
            {
                Xunit.Assert.True(none, string.Format(
        ...
        public static void Any<T>(IEnumerable<T> actual, Func<T, bool> predicate)
        {
            ...
            Xunit.Assert.True(any, string.Format("No expected item was found.\nExisting items: {0}", ToString(actual)));
        }
        ...
        public static void All<T>(IEnumerable<T> actual, Func<T, bool> predicate)
        {
            ...
            if (!all)
            {
                Xunit.Assert.True(all, string.Format(
        ...
        {
            Xunit.Assert.False(true, message);
        }

        public static void Fail(string format, params object[] args)
        {
            Xunit.Assert.False(true, string.Format(format, args));
        }

        public static void Null<T>(T @object, string message = null)
        {
            Xunit.Assert.True(AssertEqualityComparer<T>.IsNull(@object), message);
        }

        public static void NotNull<T>(T @object, string message = null)
        {
            Xunit.Assert.False(AssertEqualityComparer<T>.IsNull(@object), message);
        }
        ...
        public static void ThrowsArgumentNull(string parameterName, Action del)
        {
            ...
            {
                Xunit.Assert.Equal(parameterName, e.ParamName);
            }
        }
        ...
        public static T Throws<T>(Action del, bool allowDerived = false) where T : Exception
        {
            ...
            catch (Exception ex)
            {
                ...
                Xunit.Assert.Equal(typeof(T), type);
            }
            ...
            [CallerLineNumber]int expectedValueSourceLine = 0)
        {
            ...
            {
                Xunit.Assert.True(false, GetAssertMessage(expected, actual, escapeQuotes, expectedValueSourcePath, expectedValueSourceLine));
            }
        }
        ...
        public static void AssertContainsToleratingWhitespaceDifferences(string expectedSubString, string actualString)
        {
            ...
            Xunit.Assert.Contains(expectedSubString, actualString, StringComparison.Ordinal);
        }
